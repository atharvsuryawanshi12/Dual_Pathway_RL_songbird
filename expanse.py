...
BG_sig_slope = 2.5  # 2.5 is the BG slope tuned using 2D plots
BG_sig_mid = 0
RA_sig_slope = 18 # 18 is max RA sig slope such that MC does not have skew
RA_sig_mid = 0
# need for MC is removed while tuning
...
class Environment:
    ...
    def plot_expanse(self, nos):
        ...
        for i in tqdm(range(nos)):
            ...
            mc_out, _ = self.model.forward(input)
            x[i], y[i] = mc_out[0], mc_out[1]
        plt.hist2d(x,y, density=True, bins=30)
        plt.xlim(-1, 1)
        plt.ylim(-1, 1)
        plt.colorbar()
        plt.title(f'MC histogram without sigmoid and RA slope{RA_sig_slope}')
        plt.show()
    ...
    def plot_histogram_RA(self, nos):
        ...
        for i in tqdm(range(nos)):
            self.model.W_hvc_bg = np.random.uniform(-1, 1, (self.hvc_size, self.bg_size))
            _, x[i,:] = self.model.forward(input)
