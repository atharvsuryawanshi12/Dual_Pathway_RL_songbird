...
class NN:
    ...
    def forward(self, hvc_array, parameters):
        ...
        if self.landscape == 0: self.mc = 1.25*np.dot(self.ra, self.W_ra_mc/np.sum(self.W_ra_mc, axis=0)) # outputs to +-0.50
        else: self.mc = np.dot(self.ra, self.W_ra_mc/np.sum(self.W_ra_mc, axis=0))
