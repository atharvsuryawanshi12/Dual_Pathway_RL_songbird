...
class Environment:
    ...
    def save_trajectory(self, syll):
        ...
        #         os.remove(os.path.join(save_dir, filename))
        # Save the plot
        plt.savefig(os.path.join(save_dir, f"trajectory_{self.seed}_{LEARNING_RATE_HL}.png"))
    ...
    def save_results(self, syll):
        ...
        #         os.remove(os.path.join(save_dir, filename))
        # Save the plot
        plt.savefig(os.path.join(save_dir, f"results_{self.seed}_{LEARNING_RATE_HL}.png"))
        plt.close()  # Close the plot to avoid memory leaks
    ...
    def save_dw_day(self, syll):
        if ANNEALING:
            ...
            #         os.remove(os.path.join(save_dir, filename))
            # Save the plot
            plt.savefig(os.path.join(save_dir, f"dw_{self.seed}_{LEARNING_RATE_HL}.png"))
            plt.close()  # Close the plot to avoid memory leaks         
...
for j in range(len(val_array)):
    LEARNING_RATE_HL = val_array[j]
    returns1 = np.zeros((len(seeds), N_SYLL))
    for i in tqdm(range(len(seeds))):
        seed = seeds[i]
        returns1[i, :] = build_and_run(seed, annealing=True, plot=True)
